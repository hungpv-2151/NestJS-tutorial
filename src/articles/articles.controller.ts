...
export class ArticlesController {
  ...
  @UseGuards(RequiredAuthGuard) @Get('feed') feed(@Query() query: ArticleQueryDto, @CurrentPrincipal() principal: Principal) { return this.articles.feed(query, principal.id); }
  ...
  @UseGuards(RequiredAuthGuard) @Post(':slug/favorite') @HttpCode(200) favorite(@Param('slug') slug: string, @CurrentPrincipal() principal: Principal) { return this.articles.favorite(slug, principal.id, true); }
  @UseGuards(RequiredAuthGuard) @Delete(':slug/favorite') unfavorite(@Param('slug') slug: string, @CurrentPrincipal() principal: Principal) { return this.articles.favorite(slug, principal.id, false); }
