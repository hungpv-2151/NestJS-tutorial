...
class ArticleFields {
  @IsString() @IsNotEmpty({ message: "can't be blank" }) @MaxLength(200) title!: string;
  @IsString() @IsNotEmpty({ message: "can't be blank" }) @MaxLength(500) description!: string;
  @IsString() @IsNotEmpty({ message: "can't be blank" }) @MaxLength(50_000) body!: string;
...
class UpdateFields { @IsOptional() @IsString() @IsNotEmpty({ message: "can't be blank" }) @MaxLength(200) title?: string; @IsOptional() @IsString() @IsNotEmpty({ message: "can't be blank" }) @MaxLength(500) description?: string; @IsOptional() @IsString() @IsNotEmpty({ message: "can't be blank" }) @MaxLength(50_000) body?: string; @ValidateIf((_object, value) => value !== undefined) @IsArray() @ArrayMaxSize(20) @IsString({ each: true }) @MaxLength(50, { each: true }) tagList?: string[]; }
