 ...
 *
 * Pattern source: shipped .codex/.skignore baseline + optional per-project
 * override at <git-root>/.codex/.skignore. Both follow gitignore spec via the
...
try {
  ...
  try {
    ...
    const claudeDir = path.dirname(__dirname); // hooks/ → .codex/
