 ...
 * files (.md/.mdx/…) carry SEMANTIC edges that need an LLM re-extraction, which nothing
 * in the kit runs automatically — `graphify update .` does NOT touch docs, so neither the
 * hook nor `rebuild-spec`'s preflight adds doc edges. Only `tkm:index-docs` does, on user
 * request. This module powers the kit's advisory "your docs are stale" nudge — it points
 * at that skill and never spends tokens itself.
 *
...
function buildNudge(paths) {
  ...
  return (
    ...
    `edges, run \`/tkm:index-docs\` — it is the only skill that re-extracts docs ` +
    `(\`graphify update .\` covers code only):\n${list}${more}`
  );
}
