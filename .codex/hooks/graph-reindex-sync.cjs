 ...
 * overlapping / rapid re-runs). Doc/semantic re-extraction (LLM) is NOT done here, and no
 * other part of the kit runs it automatically either — `tkm:index-docs` owns it, on user
 * request. This hook only NUDGES when docs changed since the graph was built.
 *
